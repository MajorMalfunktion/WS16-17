...
-- which returns a Graph with a function,
-- that has an equal subset to a prior function
-- therefore it determins whether it's cyclic
isCyclic :: Eq a => Graph a -> Bool
isCyclic graph = or $ map self nodes
        where
-- generates closure = succ' 
-- as in closureF and uses closure in the self function
...
--    = saveDiv 
--          (fromJust $ safeSqrt x) 
--          (fromJust $ safeSqrt (fromJust $ saveDiv y z))
-- -safer- version:
f x y z 
    = case (or [isNothing f1, isNothing f2]) of
        True    -> Nothing
